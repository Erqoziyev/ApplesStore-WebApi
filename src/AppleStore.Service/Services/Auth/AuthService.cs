...
using AppleStore.Domain.Exceptions.Auth;
...
#pragma warning disable
    public async Task<(bool Result, int CachedMinutes)> RegisterAsync(RegistorDto dto)
    {
        ...
        if (_memoryCache.TryGetValue(REGISTER_CACHE_KEY + dto.PhoneNumber, out RegistorDto cachedRegisterDto))
        {
    ...
    {
        if (_memoryCache.TryGetValue(REGISTER_CACHE_KEY + phone, out RegistorDto registerDto))
        {
    ...
    private async Task<bool> RegisterToDatabaseAsync(RegistorDto registerDto)
    {
        var user = new User();
